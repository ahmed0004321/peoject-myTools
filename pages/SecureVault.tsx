        ...
        <div className="max-w-xl mx-auto space-y-6">
            <Card>
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-2xl font-bold text-slate-800">Secure File Vault</h2>
                    <p className="text-slate-500 mt-2">AES-256 Client-Side Encryption (.mtv format)</p>
                </div>

                <div className="flex rounded-lg bg-slate-100 p-1 mb-8">
                    <button
                        onClick={() => { setMode('encrypt'); setFile(null); setStatus(null); setError(null); }}
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all flex items-center justify-center gap-2 ${mode === 'encrypt' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}
                    >
                        <Lock size={16} /> Encrypt
                    </button>
                    <button
                        onClick={() => { setMode('decrypt'); setFile(null); setStatus(null); setError(null); }}
                        className={`flex-1 py-2 text-sm font-medium rounded-md transition-all flex items-center justify-center gap-2 ${mode === 'decrypt' ? 'bg-white shadow-sm text-indigo-600' : 'text-slate-500'}`}
                    >
                ...
                <div className="space-y-6">
                    <div
                        className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-colors ${file ? 'border-indigo-400 bg-indigo-50' : 'border-slate-200 hover:border-indigo-300'}`}
                    ...
                    >
                        ...
                        {file ? (
                            <div className="flex flex-col items-center">
                                ...
                                <p className="font-medium text-slate-700 break-all">{file.name}</p>
                                <p className="text-xs text-slate-400 mt-1">{(file.size / 1024).toFixed(1)} KB</p>
                                <div className="flex gap-2 mt-4">
                                    <span className="text-xs text-indigo-600 font-bold uppercase py-1 px-2 bg-indigo-100 rounded">Selected</span>
                                    <button
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase">Passphrase</label>
                        <div className="relative">
                            <input
                                type={showPassword ? "text" : "password"}
                                value={password}
                                onChange={(e) => setPassword(e.target.value)}
                                className="w-full p-3 pr-10 rounded-lg border border-slate-200 outline-none focus:border-indigo-500 transition-colors"
                    ...
                    {status && (
                        <div className="p-4 bg-emerald-50 text-emerald-700 rounded-xl text-center text-sm font-medium border border-emerald-100">
                            {status}
                        </div>
                    )}

                    {error && (
                        <div className="p-4 bg-red-50 text-red-700 rounded-xl text-center text-sm font-medium border border-red-100 flex items-center justify-center gap-2">
