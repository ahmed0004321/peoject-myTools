    ...
    return (
        <div className="max-w-xl mx-auto">
            ...
            <div className="mb-8 text-center">
                <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Advanced Converter</h1>
                <p className="text-slate-500">Fast, offline conversions for standard units.</p>
            </div>

            <Card className="p-0 overflow-hidden border-0 shadow-xl shadow-indigo-100">
                <div className="flex overflow-x-auto border-b border-slate-100 bg-slate-50/50 no-scrollbar">
                    {categories.map((tab) => (
                        <button
                            ...
                            className={`min-w-[80px] flex-1 py-4 flex flex-col items-center gap-1 text-xs font-bold uppercase tracking-wider transition-all duration-300 ${category === tab.id ? 'bg-white text-indigo-600 shadow-sm transform scale-105 rounded-t-lg border-t-2 border-indigo-500' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100/50'}`}
                        >
                ...
                <div className="p-8 space-y-8 bg-white/80 backdrop-blur-md">
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wide">Enter Value</label>
                        <input
                            type="number"
                            value={val}
                            onChange={(e) => setVal(parseFloat(e.target.value))}
                            onFocus={(e) => e.target.select()}
                            className="w-full text-5xl font-light text-slate-800 outline-none bg-transparent border-b border-slate-200 focus:border-indigo-500 py-2 transition-colors placeholder:text-slate-300"
                            placeholder="0"
                        />
                    </div>
                    ...
                    <div className="grid grid-cols-[1fr,auto,1fr] gap-4 items-center">
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className="w-full appearance-none bg-slate-50 border border-slate-200 rounded-xl p-4 pr-8 outline-none focus:ring-2 focus:ring-indigo-100 font-medium text-slate-700 transition-all cursor-pointer text-sm"
                                >
                        ...
                        <div className="text-slate-300 pt-6 flex justify-center">
                    ...
                    <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-2xl p-8 text-white text-center shadow-lg shadow-indigo-200 mt-6 relative overflow-hidden group">
