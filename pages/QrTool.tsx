    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8 text-center">
                <h1 className="text-3xl font-bold text-[var(--text-primary)] tracking-tight">QR Code Master</h1>
            ...
            <div className="flex justify-center gap-4 mb-8">
                ...
                <button
                    ...
                    className={`px-8 py-3 rounded-full font-bold flex items-center gap-2 transition-all duration-300 ${activeTab === 'scan' ? 'bg-indigo-600 dark:bg-indigo-500 text-white shadow-lg shadow-indigo-200 dark:shadow-indigo-900/40 transform scale-105' : 'bg-inset text-[var(--text-secondary)] border border-[var(--border-color)] hover:bg-white/5'} `}
                >
                        ...
                        <div className="p-8 border-r border-[var(--border-color)] flex flex-col gap-6 bg-inset">
                            <div className="relative flex-grow flex flex-col">
                                ...
                                <textarea
                                    ...
                                    className="w-full flex-grow p-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-xl outline-none focus:border-indigo-400 dark:focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50/10 dark:focus:ring-indigo-900/10 resize-none transition-all shadow-sm text-[var(--text-primary)]"
                                />
                                <Type className="absolute right-4 bottom-4 text-slate-300 dark:text-slate-600 pointer-events-none" size={20} />
                            </div>
                            <p className="text-xs font-semibold text-center text-slate-400 dark:text-slate-600 uppercase tracking-wide">
                    ...
                    <div className="p-10 flex flex-col items-center justify-center h-full min-h-[450px]">
                        {!scannedResult ? (
                            ...
                            <div className="w-full max-w-md text-center space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div>
                                    <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Scan Successful!</h3>
                                    <p className="text-[var(--text-secondary)]">We found the following data:</p>
                                </div>

                                <div className="p-5 bg-inset rounded-xl border border-[var(--border-color)] break-all text-[var(--text-primary)] font-mono text-sm relative group cursor-pointer hover:bg-white/5 transition-colors" onClick={() => navigator.clipboard.writeText(scannedResult)}>
                                    {scannedResult}
